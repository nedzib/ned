...
- For visual changes, verify by opening `index.html` directly in a browser or serving the repo root with `python3 -m http.server`. There is no automated verification to rely on, so report the manual checks you did.
